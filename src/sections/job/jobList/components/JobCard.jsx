...
const JobCard = ({ image, company, location, salary, name, area, id }) => {
  return (
    <Link href={`/job/${id}`} className="border p-2 rounded-md border-gray-200 flex flex-col gap-2 hover:shadow-lg transition-all duration-200 lg:p-8 lg:gap-6">
      <div className="flex items-center gap-2">
        <img src={image} alt="company" className="size-14 rounded-md" />
        <div className="flex flex-col gap-1 text-sm">
          <p className="font-semibold lg:text-base">{company}</p>
